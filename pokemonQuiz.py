...
class Game:
    ...
    def run_quiz(self, pokemon_range: list):
        ...
        for index, item in enumerate(data[pokemon_range[0] + 1 : pokemon_range[1] + 1], start=pokemon_range[0] + 1):
            skip = True
            if item["name"] in listed_pokemon:
                skip = False

            pokemon_set = self.get_pokemon_set(index, data)
            while skip:
                os.system("clear")
                self.print_listed_pokemon(listed_pokemon)
    ...
    def get_pokemon_set(self, index: int, data: list):
